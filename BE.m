...
Nsym = 200;      % nb symboles OFDM simulés (augmente pour lisser la courbe)
...
for ii = 1:length(EbN0dB)
    ...
    Xf = zeros(N, Nsym);
    Xf(dataIdx,:) = reshape(symb, Nu, Nsym);
    ...
    Yr = reshape(y, N+Ncp, Nsym);
    yt = Yr(Ncp+1:end,:);
    Yf = fft(yt, N, 1);
    
    %% ----- 8) Egalisation ZF (par sous-porteuse) -----
    % Evite division par zéro (cas rare si H[k] ~ 0)
    Xhat_ZF = Yf ./ Heff;                          % (N x Nsym)

    %% ----- 9) Egalisation MMSE (optionnel) -----
    % snr = Es/N0 sur une sous-porteuse utile (approx). Ici Es ≈ 1 (QPSK puissance 1)
    % Pne est variance complexe du bruit sur y(t) ; en OFDM, la variance bruit FFT reste Pne (si FFT unitaire au sens énergie)
    % -> on utilise une forme simple: W = conj(H)/( |H|^2 + Pne )
